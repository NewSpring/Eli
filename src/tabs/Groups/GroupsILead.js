...
import {
  compose, mapProps, withProps, branch, renderNothing,
} from 'recompose';
...
const isLeader = (person, leaders) => person
  && Array.isArray(leaders)
  && leaders.length
  && Boolean(leaders.filter(x => x.person.id === person.id).length);

const getLeaders = group => group && group.members && group.members.filter(x => x.role.toLowerCase() === 'leader');
